...
class RecomendadorContenido():
  ...
  # Función que realiza el filtrado por contenido para recomendar libros a un usuario específico
  def rec_cont(self,con, usuario):
    # Lee datos de libros desde la base de datos
    libros = pd.read_sql_query('select * from libros', con)
...
class SistemaRecomendacionHibrido():
  ...
  def __init__(self):
    self.sistema_colaborativo = RecomendadorColaborativo()
    self.sistema_contenido = RecomendadorContenido()

  def get_recomendations(self,con,usuario):
    pesoContenido = 0.4
    pesoColaborativo = 0.6

    listaCon = self.sistema_contenido.rec_cont(con,usuario)
    listaCol = self.sistema_colaborativo.rec_col(con,usuario)
...
usuario = '1152065'
print("Recomendaciones de contenido para el usuario", usuario + ":")

query_existencia_usuario = f'SELECT COUNT(*) FROM calificacion WHERE codigo = "{usuario}"'
existencia_usuario = con.execute(query_existencia_usuario).fetchone()[0]

if existencia_usuario > 0:
    sistema = SistemaRecomendacionHibrido()
    recomendaciones_contenido = sistema.get_recomendations(con, usuario)
    for libro, similitud in recomendaciones_contenido:
        print(f"- Libro: {libro}, Similitud: {similitud}")

    lista_recomendaciones = [{"Libro": libro, "Similitud": similitud} for libro, similitud in recomendaciones_contenido]
else:
    print(f"El usuario {usuario} no esta perfilado.")
